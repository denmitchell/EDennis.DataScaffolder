...
namespace EDennis.DataScaffolder {
    ...
    public partial class MainForm : Form {
        ...
        private void btnStart_Click(object sender, EventArgs e) {
            ...
            using (OpenFileDialog openFileDialog = new OpenFileDialog()) {
                ...
                if (openFileDialog.ShowDialog() == DialogResult.OK) {
                    ...
                    nmspace = GetNamespace(projectPath, destinationPath);
                    ...
                    btnStart.Location = new Point(btnStart.Location.X - 50, btnStart.Location.Y);
        ...
        private void ScaffoldFileStart(StreamWriter sw, string nmspace) {
            sw.WriteLine("// file generated by EDennis.DataScaffolder");
            sw.WriteLine("// using connection string(s) from appsettings.json");
            sw.WriteLine($"namespace {nmspace} {{");
        }
...
@"select table_name
	...
	order by table_name;";
        ...
        /// <summary>
        /// Determine the best namespace for the generated .cs file, based
        /// upon either an existing namespace or the destination path.
        /// </summary>
        /// <param name="projectPath">project root directory</param>
        /// <param name="destinationPath">destination folder of generated .cs file</param>
        ...
        private string GetNamespace(string projectPath, string destinationPath) {
            ...
            //first, look for a Models folder
            if (destinationPath.EndsWith("Models")) {
                
                //find an arbitrary class file
                var files = dInfo.GetFiles("*.cs");
                if (files.Count() > 0) {
                    var classFilePath = files[0].FullName;

                    //read in the code text, and extract namespace
                    var code = File.ReadAllText(classFilePath);
                    var pattern = "^\\s*(?:namespace)\\s+([A-Za-z0-9_.]+)\\s*{";
                ...
                //if there are no class files yet, use the default namespace
                return $"{dInfo.Parent.Name}.Models";
            } else {
                //if there is not Models folder yet, use the project root
                return $"{dInfo.Name}";
            }            
        }
